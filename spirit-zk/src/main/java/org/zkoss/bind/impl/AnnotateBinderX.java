...
import org.zkoss.bind.sys.debugger.impl.info.*;
...
/**
 * <code>AnnotateBinder</code>扩展类
 *
 * @author junjie
 */
public class AnnotateBinderX extends AnnotateBinder {

    private static final Logger _log = LoggerFactory.getLogger(AnnotateBinderX.class);
    ...
    private static final Map<Class<?>, List<Method>> _initMethodCache
        = new CacheMap<>(600, CacheMap.DEFAULT_LIFETIME); //class,list<init method>
    private static final Map<Class<?>, Map<String, CachedItem<Method>>> _commandMethodCache
        = new CacheMap<>(200, CacheMap.DEFAULT_LIFETIME); //class,map<command, null-able command method>¬
    ...
    private static final CachedItem<Method> NULL_METHOD = new CachedItem<>(null);
    ...
    public AnnotateBinderX() {
        this(null, null);
    }

    public AnnotateBinderX(String qname, String qscope) {
        ...
        for (Method m : getViewModel().getClass().getDeclaredMethods()) {
            ...
            if (annomm != null) {
                if (_matchMediaValues == null)
                    _matchMediaValues = new HashMap<>(6);
    ...
    private class QueueListener implements org.zkoss.zk.ui.event.EventListener<Event>, Serializable {
        ...
        public void onEvent(Event event) throws Exception {
            ...
            if (event instanceof PropertyChangeEvent) {
                ...
                AnnotateBinderX.this.doPropertyChange(evt.getBase(), evt.getProperty());
            } else if (event instanceof GlobalCommandEvent) {
                final GlobalCommandEvent evt = (GlobalCommandEvent) event;
                final Set<Property> notifys = new LinkedHashSet<>();
                AnnotateBinderX.this.doGlobalCommand(_rootComp, evt.getCommand(), evt.getArgs(), notifys);
    ...
    public Map<Component, Map<String, List<Binding>>> getAllBindings() {
    ...
    private class CommandEventListener implements org.zkoss.zk.ui.event.EventListener<Event>, Serializable, Deferrable {
        ...
        private void onEvent0(Event event) throws Exception {
            ...
            if (_prompt) {
                promptResult = AnnotateBinderX.this.doSaveEvent(comp, event, notifys); //save on event
            }
            ...
            if (_commandBinding != null) {
                ...
                Map<String, Object> implicit = _implicitContributor.contirbuteCommandObject(AnnotateBinderX.this,
                ...
                if (!Strings.isEmpty(command)) { //avoid the execution of a empty command.
                    ...
                    final Map<String, Object> args = BindEvaluatorXUtil.evalArgs(eval, comp, _commandBinding.getArgs(),
                        implicit);
                    cmdResult = AnnotateBinderX.this.doCommand(comp, _commandBinding, command, event, args, notifys);
                }
            }
            ...
            if (_prompt && promptResult) {
                ...
                AnnotateBinderX.this.doLoadEvent(comp, event); //load on event
            }
            ...
            if (cmdResult == COMMAND_SUCCESS && _globalCommandBinding != null) {
                ...
                if (!Strings.isEmpty(command)) { //avoid the execution of a empty command.
                    ...
                    if (_implicitContributor != null) {
                        implicit = _implicitContributor.contirbuteCommandObject(AnnotateBinderX.this, _commandBinding,
                            event);
                    }
    ...
    private class VMsgsChangedListener implements org.zkoss.zk.ui.event.EventListener<Event>, Serializable {
    ...
    public int sendCommand(String command, Map<String, Object> args) {
        checkInit();
        final Set<Property> notifys = new HashSet<>();
        Event evt = null;
        //ZK-3133
    ...
    private void doGlobalCommandExecute(Component comp, String command, Map<String, Object> commandArgs,
        ...
        try {
            ...
            if (method != null) {
                ...
                if (sannt != null) {
                    Set<Property> properties = new LinkedHashSet<>(5);
        ...
        try {
            ...
            ValidationHelper vHelper = new ValidationHelper(this, new ValidationHelper.InfoProvider() {
                ...
                public BindingKey getBindingKey(Component comp, String attr) {
                    return this.getBindingKey(comp, attr);
                }
            });
    ...
    public void removeBindings(Component comp, String key) {
        ...
        final Set<Binding> removed = new HashSet<>();
    ...
    private Map<String, Object> _dynamicAttrs = new HashMap<String, Object>(5) {
        ...
        public Object put(final String key, Object value) {
            ...
            AnnotateBinderX.this.postCommand(key, Collections.singletonMap("", value));
            return oldValue;
        }
    };
    ...
    public ComponentActivationListener setActivator(Component comp) {
        if (_rootComp == comp && !_rootComp.hasAttribute(ACTIVATOR)) {
            ComponentActivationListener _lsnr = new Activator();
            _rootComp.setAttribute(ACTIVATOR, _lsnr); // keep only one instance in root comp
            return _lsnr;
        }
    ...
    private void subscribeQueue(String quename, String quescope, org.zkoss.zk.ui.event.EventListener<Event> listener) {
        EventQueue<Event> que = EventQueues.lookup(quename, quescope, true);
        que.subscribe(listener);
    }

    private void unsubscribeQueue(String quename, String quescope, org.zkoss.zk.ui.event.EventListener<Event> listener) {
    ...
    private boolean isSubscribed(String quename, String quescope, org.zkoss.zk.ui.event.EventListener<Event> listener) {
        EventQueue<Event> que = EventQueues.lookup(quename, quescope, false);
    ...
    private class DeferredActivator implements ExecutionInit, Serializable {
        ...
        public void init(Execution exec, Execution parent) throws Exception {
            ...
            AnnotateBinderX.this.didActivate();
        }
    }
